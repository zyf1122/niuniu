...
export default {
  ...
  created(){
    ...
    // 创建一个空数组存储书架
    // localEvent.StorageSetter("bookshelf",[])
  },
  methods:{
    ...mapActions([
        'setBook',
    ...
    readingFn:function(){
        ...
        // 更新书本内容
        this.setCatalog(1);
        // console.log(localEvent.StorageGetter('book'),this.introduce.id)
        // 存储在最近阅读中
    ...
    likeFn:function(index){
        this.$router.push({path:'/introducelike',query:{id:this.likeArr[index].id}});
        // location.reload();
        // this.$router.go(0);
    ...
    .header
        ...
        span
        	span
        		...
        		background-position pxtorem(-66px) pxtorem(0)
    .detail
        overflow hidden
